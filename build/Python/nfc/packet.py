...
def v2_decode(v2_bytes):
    ...
    dic['expiration_date'] = ndays_to_utc(abs(items[7]))
...
def v2_encode(card_dic):
    ...
    expiration_ndays = utc_to_ndays(card_dic['expiration_date'])
...
def v2_encode_date_fn(atime_t, clean, error, tzinfo):
    ...
    encoded_date = utc_to_ndays(noon_t)
...
def v2_decode_date_bits(encoded_date):
    ...
    decoded_t = ndays_to_utc(encoded_date & V2_DAYS_MASK)
...
def v2_encode_enforce(dic):
    val = 0
    for key in V2_ENFORCE_BITS.keys():
...
def v2_defaults(cartridge_type):
    defaults = {
        ...
        'type': 1,
        'serial_number': '0000000001',
        'expiration_date': dt_to_time_t(
            dt.datetime.now(pytz.UTC) + relativedelta(years=1)),
        'guaranteed_injections': 100,
        'max_injections': 200,
